...
def new_node_connection(request):
	...
	global node_id, num_connections, node_connections, node_server_connections

	if (new_connection > len(node_connections) - 1):
		added_nodes = [new_connection - len(node_connections) + 1]
		for i in range(new_connection - len(node_connections) + 1):
			#node_connection = httplib.HTTPConnection("127.0.0.1")
			#node_server_connections.append(node_connection)
			node_server_connections.append('')
			#node_server_connections = np.hstack((node_server_connections,np.zeros(new_connection - len(node_connections) + 1)))	
		node_connections = np.hstack((node_connections,np.zeros(new_connection - len(node_connections) + 1)))	
	if (node_connections[new_connection] == 0):
		node_connections[new_connection] = 1
		num_connections = num_connections + 1		
		
		node_connection_addr = "127.0.0.1:" + str(6000 + new_connection)
 		node_connection = httplib.HTTPConnection(node_connection_addr)
		node_server_connections[new_connection] = node_connection
...
def client_task():
	...
	global node_id, num_connections, node_connection, energy_out, node_server_connections
	...
	if (len(connections) > 0):
		for c in range(0, len(connections)):
			...
			conn.request("PUT", "", params, headers)
			r1 = conn.getresponse()
			if (node_connections[connections[c]] == 0):
				node_connections[connections[c]] = 1
				num_connections = num_connections + 1
				# add the connections
				node_connection_addr = "127.0.0.1:" + str(6000 + connections[c])
 				node_connection = httplib.HTTPConnection(node_connection_addr)
 				node_server_connections[connections[c]] = node_connection
	'''		
	#main loop
	while (1):
...
if __name__ == '__main__':
	...
	r1 = conn.getresponse()
	node_id = int(r1.read())
	node_connections = np.zeros(node_id + 1)
